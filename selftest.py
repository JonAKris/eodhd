...
def _inst_ctx(row_or_none, floors=None):
    rows = [] if row_or_none is None else [row_or_none]
    return Context(fetch_all=lambda sql, params=None: rows, floors=floors or Floors())
...
def test_institutional() -> bool:
    ...
    asof = date(2026, 4, 10)
    ok = True
    print("institutional_flow (snapshot -> prospective-only):")
